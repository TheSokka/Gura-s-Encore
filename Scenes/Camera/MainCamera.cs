...
public partial class MainCamera : Node {
    ...
    public override void _Input(InputEvent @event) {
        if (@event is InputEventMouseMotion mouseInput) {
            ...
            _cameraRotationDegrees.X +=
                Mathf.DegToRad(-mouseInput.Relative.Y * (_mouseSensitivity * (_inverted ? -1 : 1)));
            _cameraRotationDegrees.Y +=
                Mathf.DegToRad(mouseInput.Relative.X * (_mouseSensitivity * (_inverted ? -1 : 1)));
